...
class GroceryHelpersAPI:
    ...
    def search(self, term, timeout=10, follow_first_link=False):
        ...
        while len(items) == 0 and time.time() - start_time < 10:
            items = self._driver.find_elements_by_class_name('product-tile-group__item')
            product_data = [json.loads(item.find_element_by_class_name('product-tracking').get_attribute('data-track-products-array')) for item in items]
            links = self._driver.find_elements_by_class_name('product-tile__details__info__name__link')
            
            if len(self._driver.find_elements_by_class_name('search-no-results__section-title')):
        ...
        df['previouslyPurchased'] = ['Previously Purchased' in item.find_element_by_class_name('product-tile__eyebrow').text for item in items]
        df['link'] = [link.get_attribute('href') for link in links]    
        df['categories'] = [urllib.request.unquote(link).replace('-', ' ').split('/')[4:-2] for link in df['link']]
        ...
        for item in items:
            ...
            for ul in item.find_elements_by_tag_name('ul'):
                for li in ul.find_elements_by_tag_name('li'):
                    data = [span.text for span in li.find_elements_by_tag_name('span')]
    ...
    def add_product_to_current_order(self, link, quantity=1, timeout=10):
        ...
        button = [button for button in buttons if button.text == 'ADD'][0]
        button.click()
    ...
    def get_itemized_order_history(self, timeout=10):
        ...
        for i, link in enumerate(df_orders['link']):
            ...
            while time.time() - start_time < timeout:
                try:
                    product_descriptions = self._driver.find_elements_by_class_name('order-history-details-products__product__info__name')
                except NoSuchElementException:
                    pass

            product_descriptions = [product_description.text for product_description in product_descriptions]

            product_skus = self._driver.find_elements_by_class_name('order-history-details-products__product__info__code')
            product_skus = [product_sku.text for product_sku in product_skus]

            product_quantities = self._driver.find_elements_by_class_name('order-history-details-products__product__quantity')
            product_quantities = [product_quantity.text for product_quantity in product_quantities]

            product_prices = self._driver.find_elements_by_class_name('order-history-details-products__product__price')
            product_prices = [float(product_price.text[1:]) for product_price in product_prices]
